const hitungDataBulan = () => {let latText = document.getElementById("latitude"); let longText = document.getElementById("longitude");navigator.geolocation.getCurrentPosition((position) => {let lat = position.coords.latitude;let long = position.coords.longitude;latText.innerText = lat.toFixed(3);longText.innerText = long.toFixed(3);});let ltd = longText.innerText - 106.000 + 106.000;let ltd2 = ltd.toFixed(3);let tahunmajmuah = document.querySelector("#tahunmajmuah").value; let totaltahunmajmuah =  parseFloat(tahunmajmuah);
const dataMajmuah = [[1410, 161.900], [1420, 33.983], [1430, 74.067], [1440, 114.150], [1450, 154.233], [1460, 26.317], [1470, 66.400], [1480, 106.483], [1490, 146.566], [1500, 18.649], [1510, 58.732], [1520, 98.815], [1530, 138.898], [1540, 10.981], [1550, 51.064]];const dataHissohSinin = [[1410, 197.550], [1420, 278.050], [1430, 358.550], [1440, 79.050], [1450, 159.550], [1460, 240.050], [1470, 320.550], [1480, 41.050], [1490, 121.550], [1500, 202.050], [1510, 282.550], [1520, 3.050], [1530, 83.550], [1540, 164.050], [1550, 244.550]];const dataWasatSinin = [[1410, 162.000], [1420, 54.800], [1430, 307.600], [1440, 200.400], [1450, 93.200], [1460, 346.000], [1470, 238.800], [1480, 131.600], [1490, 24.400], [1500, 277.200], [1510, 170.000], [1520, 62.800], [1530, 315.600], [1540, 208.400], [1550, 101.200]];const dataKhosohSinin = [[1410, 322.217], [1420, 180.217], [1430, 38.217], [1440, 256.217], [1450, 114.217], [1460, 332.217], [1470, 190.217], [1480, 48.217], [1490, 266.217], [1500, 124.217], [1510, 342.217], [1520, 3.050], [1530, 83.550], [1540, 164.050], [1550, 244.550]];const dataMarkazSinin = [[1410, 59.833], [1420, 312.500], [1430, 205.167], [1440, 97.833], [1450, 350.500], [1460, 243.167], [1470, 135.833], [1480, 28.500], [1490, 281.167], [1500, 173.833], [1510, 66.500], [1520, 319.167], [1530, 211.834], [1540, 104.501], [1550, 357.168]];let alamahsininmajmuah = dataMajmuah.find(([batas]) => tahunmajmuah <= batas)?.[1] || 0;let hissohsininmajmuah = dataHissohSinin.find(([batas]) => tahunmajmuah <= batas)?.[1] || 0;let wasatsininmajmuah = dataWasatSinin.find(([batas]) => tahunmajmuah <= batas)?.[1] || 0;let khosohsininmajmuah = dataKhosohSinin.find(([batas]) => tahunmajmuah <= batas)?.[1] || 0;let markazsininmajmuah = dataMarkazSinin.find(([batas]) => tahunmajmuah <= batas)?.[1] || 0;let tahunmabsutoh = document.querySelector("#tahunmabsutoh").value;let totaltahunmabsutoh = parseFloat(tahunmabsutoh);const dataAlamahSininMabsutoh = [[1, 104.800], [2, 41.617], [3, 146.417], [4, 83.233], [5, 20.033], [6, 124.850], [7, 61.650], [8, 166.467], [9, 103.267], [10, 40.083]];const dataHissohSininMabsutoh = [[1, 8.050], [2, 16.100], [3, 24.150], [4, 32.200], [5, 40.250], [6, 48.300], [7, 56.350], [8, 64.400], [9, 72.450], [10, 80.500]];const dataWasatSininMabsutoh = [[1, 349.267], [2, 338.567], [3, 327.833], [4, 317.117], [5, 306.400], [6, 295.683], [7, 284.967], [8, 274.233], [9, 263.517], [10, 252.800]]; 
   const dataKhosohSininMabsutoh = [[1, 309.800], [2, 259.600], [3, 209.400], [4, 159.200], [5, 109.000], [6, 58.800], [7, 8.600], [8, 318.400], [9, 268.200], [10, 218.000]];const dataMarkazSininMabsutoh = [[1, 349.267], [2, 338.533], [3, 327.800], [4, 317.067], [5, 306.333], [6, 295.600], [7, 284.867], [8, 274.133], [9, 263.400], [10, 252.667]];let alamahsininmabsutoh = dataAlamahSininMabsutoh.find(([batas]) => totaltahunmabsutoh <= batas)?.[1] || "0"; let hissohsininmabsutoh = dataHissohSininMabsutoh.find(([batas]) => totaltahunmabsutoh <= batas)?.[1] || "0"; 
   let wasatsininmabsutoh = dataWasatSininMabsutoh.find(([batas]) => totaltahunmabsutoh <= batas)?.[1] || "0";let khosohsininmabsutoh = dataKhosohSininMabsutoh.find(([batas]) => totaltahunmabsutoh <= batas)?.[1] || "0";let markazsininmabsutoh = dataMarkazSininMabsutoh.find(([batas]) => totaltahunmabsutoh <= batas)?.[1] || "0";let bulanhijriyah = document.querySelector("#bulanhijriyah").value; let totalbulan = parseFloat(bulanhijriyah); const dataAlamah = [[1, 68.067], [2, 0.000], [3, 36.733], [4, 73.467], [5, 110.200], [6, 146.933], [7, 15.667], [8, 52.400], [9, 89.133], [10, 125.867], [11, 162.600], [12, 31.333]];const dataHissoh = [[1, 337.383], [2, 0.000], [3, 30.667], [4, 61.333], [5, 92.017], [6, 122.683], [7, 153.350], [8, 184.017], [9, 214.700], [10, 245.367], [11, 276.050], [12, 306.717]];const dataWasat = [[1, 320.167], [2, 0.000], [3, 29.100], [4, 58.217], [5, 87.317], [6, 116.433], [7, 145.533], [8, 174.633], [9, 203.750], [10, 232.850], [11, 261.950], [12, 291.067]];const dataKhosoh = [[1, 283.983], [2, 0.000], [3, 25.817], [4, 51.633], [5, 77.433], [6, 103.267], [7, 129.083], [8, 154.900], [9, 180.717], [10, 206.533], [11, 232.350], [12, 258.167]];const dataMarkaz = [[1, 320.167], [2, 0.000], [3, 29.100], [4, 58.217], [5, 87.317], [6, 116.433], [7, 145.533], [8, 174.633], [9, 203.750], [10, 232.850], [11, 261.950], [12, 291.067]];let alamahbulan = dataAlamah.find(([batas]) => totalbulan <= batas)?.[1] || "0";let hissohbulan = dataHissoh.find(([batas]) => totalbulan <= batas)?.[1] || "0";let wasatbulan = dataWasat.find(([batas]) => totalbulan <= batas)?.[1] || "0";let khosohbulan = dataKhosoh.find(([batas]) => totalbulan <= batas)?.[1] || "0";let markazbulan = dataMarkaz.find(([batas]) => totalbulan <= batas)?.[1] || "0";let totalAlamah = alamahsininmajmuah + alamahsininmabsutoh + alamahbulan;let hasilAkhirAlamah = totalAlamah >= 168 ? totalAlamah - 168 : totalAlamah -0;let hasilAkhirAlamah2 = hasilAkhirAlamah >= 168 ? hasilAkhirAlamah - 168 : hasilAkhirAlamah -0;let totalHissoh = hissohsininmajmuah + hissohsininmabsutoh + hissohbulan;let hasilAkhirHissoh = totalHissoh >= 360 ? totalHissoh - 360 : totalHissoh -0;let hasilAkhirHissoh2 = hasilAkhirHissoh >= 360 ? hasilAkhirHissoh - 360 : hasilAkhirHissoh -0;            function hitungHasil(total, batas) {
  return total >= batas ? total - batas : total;}
   let totalWasat123 = wasatsininmajmuah + wasatsininmabsutoh + wasatbulan;let hasilAkhirWasat = hitungHasil(totalWasat123, 360);let hasilAkhirWasat2 = hitungHasil(hasilAkhirWasat, 360);let totalKhosoh123 = khosohsininmajmuah + khosohsininmabsutoh + khosohbulan;let hasilAkhirKhosoh = hitungHasil(totalKhosoh123, 360);let hasilAkhirKhosoh2 = hitungHasil(hasilAkhirKhosoh, 360);let totalMarkaz123 = markazsininmajmuah + markazsininmabsutoh + markazbulan;let hasilAkhirMarkaz = hitungHasil(totalMarkaz123, 360);let hasilAkhirMarkaz2 = hitungHasil(hasilAkhirMarkaz, 360);let hasilAkhirAlamah3 = hasilAkhirAlamah2.toFixed(3);let hasilAkhirHissoh3 = hasilAkhirHissoh2.toFixed(3);let hasilAkhirWasat3 = hasilAkhirWasat2.toFixed(3);let hasilAkhirKhosoh3 = Math.round(hasilAkhirKhosoh2);let hasilAkhirMarkaz3 = Math.round(hasilAkhirMarkaz2); 
...
let index = Math.ceil(hasilAkhirKhosoh3);if (index < 0) index = 0;if (index >= lookupTadilKhosoh.length) index = lookupTadilKhosoh.length - 1;tadilkhosoh = lookupTadilKhosoh[index];const lookupTadilMarkaz = [
...
let mq3 = (mq1 <= 2) ? 'Utara' : (mq1 <= 8 ? 'Selatan' : 'Utara');let mq3a = (mq1 <= 2) ? 'Selatan' : (mq1 <= 8 ? 'Utara' : 'Selatan');let mq4 = Math.round(muqowwamsyamsi);
...
<table class="table-harokat">
  <tr class="tr-harokat">
   <th class="th-harokat">Nama</th><th class="th-harokat">Hitungan</th> <th class="th-harokat"></th></tr><tr class="tr-harokat"> <td class="td-harokat"> Ta'dil Khosoh </td> <td class="td-harokat"> ${tadilkhosoh} </td>	 <td class="td-harokat"> </td>	</tr> <tr class="tr-harokat"><td class="td-harokat"> Ta'dil Markaz </td>  <td class="td-harokat"> ${tadilmarkaz} </td>  
    <td class="td-harokat"> </td>	</tr> <tr class="tr-harokat"><td class="td-harokat"> Bu'du Goer Muadal </td><td class="td-harokat"> ${buduGoerMuadal2}</td> <td class="td-harokat"> </td>	</tr> </tr>
    <tr class="tr-harokat"><td class="td-harokat"> Hasilud Dorob </td> <td class="td-harokat"> ${hasildorob2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Ta'dil Wasat </td> <td class="td-harokat"> ${tadilwasat2} <td class="td-harokat"></tr> <tr class="tr-harokat"><td class="td-harokat"> Muqowwam Syamsi </td><td class="td-harokat"> ${muqowwamsyamsi2} <td class="td-harokat"></tr> <tr class="tr-harokat"><td class="td-harokat"> Buruj </td><td class="td-harokat"> ${mq1} <td class="td-harokat"></tr>
    <tr class="tr-harokat"><td class="td-harokat"> Yaqoul Ijtima Piiburj </td><td class="td-harokat"> ${mq2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Hai'atul Hilal Maailatun Ilaa </td><td class="td-harokat"> ${mq3} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Ta'dil Ayyam </td><td class="td-harokat"> ${tadilayyam} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Bu'du Muaddal </td><td class="td-harokat"> ${budumuadal2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Thul Syamsi </td><td class="td-harokat"> ${thulsyamsi2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Hissoh Sa'ah </td><td class="td-harokat"> ${hissohsaah} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Ta'dil Alamah </td><td class="td-harokat"> ${tadilalamah2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Alamah Muadalah Jkt </td><td class="td-harokat"> ${jkt3} <td class="td-harokat"></tr>        
    <tr class="tr-harokat"><td class="td-harokat"> Busur Tempat </td><td class="td-harokat"> ${ltd2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Selisih Waktu </td><td class="td-harokat"> ${selisihwaktu2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> AMB Bittatbieq </td>   <td class="td-harokat"> ${bittatbieq2} <td class="td-harokat"></tr>  <tr class="tr-harokat"> <td class="td-harokat"> Yaqoul Ijtima Lailata </td> <td class="td-harokat"> ${yaqoulijtima} <td class="td-harokat"> </tr><tr class="tr-harokat"><td class="td-harokat"> Sa'ah Ijtima Gurubiyah </td><td class="td-harokat"> ${sig2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Sa'ah Ijtima Zawaliyah </td>    <td class="td-harokat"> ${siz2} <td class="td-harokat"></tr>     <tr class="tr-harokat"><td class="td-harokat"> Minal Ijatima Ilal Gurub </td><td class="td-harokat"> ${mig2} <td class="td-harokat"></tr>  <tr class="tr-harokat"><td class="td-harokat"> Irtipa Hilal Ba'dal Gurub </td><td class="td-harokat"> ${irtipa2} <td class="td-harokat"></tr><tr class="tr-harokat"><td class="td-harokat"> Muktsul Hilal Pauqol Ufuk </td><td class="td-harokat"> ${mukstulhilal2} <td class="td-harokat"></tr>   <tr class="tr-harokat"> <td class="td-harokat"> Kamyah Ardl Qomar </td><td class="td-harokat"> ${kamyah} <td class="td-harokat"></tr> <tr class="tr-harokat"><td class="td-harokat"> Qous Nuril Hilal </td><td class="td-harokat"> ${nurilhilal2} <td class="td-harokat"></tr></table></article>`;}};
const calculate1 = () => { let {tahunmajmuah, totalbulan, tahunmabsutoh, bulanhijriyah, alamahsininmajmuah, hissohsininmajmuah, wasatsininmajmuah,khosohsininmajmuah, markazsininmajmuah, alamahsininmabsutoh, hissohsininmabsutoh,wasatsininmabsutoh, khosohsininmabsutoh, markazsininmabsutoh, namebulan, alamahbulan, hissohbulan, wasatbulan,khosohbulan, markazbulan, hasilAkhirAlamah3, hasilAkhirHissoh3, hasilAkhirWasat3, hasilAkhirKhosoh3, hasilAkhirMarkaz3,tadilkhosoh, tadilmarkaz, buduGoerMuadal2, hasildorob2, tadilwasat2, muqowwamsyamsi2, mq1, mq2, mq3, tadilayyam, budumuadal2,thulsyamsi2, tadilalamah2, hissohsaah, jkt3, ltd2, selisihwaktu2, bittatbieq2, yaqoulijtima, sig2, siz2, mig2, irtipa2, mukstulhilal2, kamyah, nurilhilal2,harimasuk, harimasuk2, harimasuk6, harimasuk7, imkanGoerImkan, imkanGoerImkan2, imkanGoerImkan3, imkanGoerImkan4, derajatirtipa, drj,thM, tjdIjt, jmsiz2, jmsiz4, jmsig2, jmsig4, jmipa2, jmipa4, jmmks2, jmmks4, qnh2, qnh4, yaqoulijtima2, ptkimkan, ikngoerikn, mq3a, } = hitungDataBulan();
        if (tahunmajmuah == 0) {
   document.querySelector("#showdata").innerHTML = `Maaf, Anda Belum Memilih Tahun Majmuah `;} 
 		else if (tahunmabsutoh == 0) {document.querySelector("#showdata").innerHTML = `Maaf, Anda Belum Memilih Tahun Mabsutoh `;}
 		else if (bulanhijriyah == 0) {document.querySelector("#showdata").innerHTML = `Maaf, Anda Belum Memilih Bulan `;}
 		else if (derajatirtipa == 0) {document.querySelector("#showdata").innerHTML = `Penting Untuk Menetukan Irtipa `;}
 		else {document.querySelector("#showdata").innerHTML = `  <table><tr><td> Awal Bulan ${bulanYangDimaksud} ${tahunYangDimaksud} H </td></tr> <tr><td> Jatuh Pada Hari ${jatuhhari}, ${thM} M</td></tr><tr><td> Ijtima Terjadi Pada ${tjdIjt} ${yaqoulijtima}</td></tr><tr><td> Jam Ijtima  ${jmsiz2}:${jmsiz4} WIB | ${jmsig2}:${jmsig4} WG </td></tr><tr><td> Ketinggian Hilal {Malam ${yaqoulijtima2}} ${jmipa2}°${jmipa4}'</td></tr><tr><td> Patokan Imkan ${ptkimkan} [${ikngoerikn}]</td></tr><tr><td> Lama Hilal diatas Ufuk ${jmmks2}:${jmmks4}</td></tr> <tr><td> Arah Hilal di ${mq3a}</td></tr><tr><td> Condongnya Hilal Miring ke ${mq3}</td></tr><tr><td> Cahaya Hilal ${qnh2}°${qnh4}</td></tr>	</table>  `;}}
